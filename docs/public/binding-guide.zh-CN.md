...
不提供 Binding 文件时，编译器直接把 AAL 审计名称作为生成程序名称，并按照当前源文件声明顺序生成本次构建使用的确定性临时 ID。HTTP 字段名称也默认使用 AAL 流程输入名称，不受 Binding 控制。

这种直接模式就是开始和运行应用的默认方式，但它不是持久身份契约：源文件重命名会改变程序名称，调整声明顺序可能改变临时 ID。
...
不要仅仅因为应用使用英文名称就增加 Binding。如果审计名称本身适合作为程序名称，也不需要持久 ID，直接模式已经完整。
...
`auditName` 用来匹配所选 AAL 源文件；`id` 用来在明确的审计名称变化中保持身份；`programName` 控制生成的 TypeScript 名称。它不会暗中重命名 HTTP 请求或响应字段；HTTP 请求名称不同时，应在 AAL 的 HTTP 入口中通过局部 `as` 明确声明。
...
AAL 名称可以直接使用且不需要持久身份时，省略 `--binding`。
