...
状态：**规划基线，AAL 与 Oracle 尚未实现**

本文件用于判断未来实现是在压缩业务语义，还是把语义转移到了编译器或 Runtime。每条冻结语义都必须在 AAL、语言标准或 Fixture 中拥有明确归属，并由对应 Oracle 覆盖。
...
下表中的“AAL 定位”是未来 `app.zh-CN.aal` 应提供的审计锚点；“标准语义”不能替代 AAL 中的业务表达。

| 编号 | 冻结规则 | AAL 定位 | 标准语义 | 计划 Oracle 用例 |
|---|---|---|---|---|
