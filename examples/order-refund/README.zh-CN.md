...
- [原子流程阅读候选版](./app.atomic-flow.zh-CN.aal)
- [冻结 Fixture](./fixture.v1.json)

## 原子流程阅读候选版

`app.atomic-flow.zh-CN.aal` 不覆盖稳定版本。它把“一个流程就是一个完整事务”作为默认语义，因而：

- 不再重复书写 `同时生效`；
- 一次性计算直接写入判断或输出；
- 退款的共同状态改变只写一次，后续步骤可以读取本流程刚产生的新状态。

稳定版为 397 行，候选版为 368 行。行数只减少 29 行，主要观察点不是压缩比例，而是退款流程是否更接近业务人员自然阅读的顺序。

候选版可以由当前编译器解析和运行，但当前 Runtime 尚未提供通用的流程级原子提交保证。它目前只用于比较阅读体验，不能替代使用 `同时生效` 的稳定版本。

可以使用同一套冻结 Oracle 比较当前可观察行为：

```bash
npm run test:order-refund:atomic-flow
```
