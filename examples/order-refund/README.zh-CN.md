...
`app.atomic-flow.zh-CN.aal` 不覆盖稳定版本。这里的“原子流程”指小型、单一职责、可组合的审计单元，不是事务边界。候选版：
...
这次改写没有追求减少总行数，而是验证首次完整审计之后，后续是否可以围绕较小的稳定语义单元做增量审计。它也暴露了一个值得观察的问题：当前 `执行 / 使用 / 得到` 的显式连接会明显增加主流程篇幅。
